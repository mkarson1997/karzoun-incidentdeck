...
void main() {
  test(
    'same-time operations use numeric revision order for one incident',
    () async {
      ...
      expect(pending.map((operation) => operation.targetRevision), <int>[
        10,
        11,
      ]);
    },
  ...
  test(
    'fresh pending work check blocks remote apply after local read',
    () async {
      final outbox = InMemorySyncOutbox();
